 ...
 * Capability is de/serialized using its {@link ISerializableCapability#deserializeCapability}/{@link ISerializableCapability#serializeCapability}
 * methods.
 * @param <C> Capability type
 */
public class SerializableCapabilityWrapperProvider<C extends ISerializableCapability> implements ICapabilitySerializable<Tag> {
	...
	public <T> LazyOptional<T> getCapability(@NotNull Capability<T> cap, @Nullable Direction side) {
		return cap == capability && side == this.side ? LazyOptional.of(() -> instance).cast() : LazyOptional.empty();
	}
